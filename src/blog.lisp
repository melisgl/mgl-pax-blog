...
(defun quotenil-rss-feed-for-current-page (categories)
  (let ((category (category-for-current-page categories)))
    (if category
        (format nil "<link rel='alternate' href='~A' ~
...
(defpost @pax-v0.4
  ...
  list](https://github.com/melisgl/mgl-pax/commits/parse-word)."""
...
(defpost @adaptive-hashing
    (:title "Adaptive Hashing"
     :tags (@tech @lisp)
     :date "2025-05-02")
  """At [ELS 2024](https://www.european-lisp-symposium.org/2024/),
  I gave a [talk on adaptive
  hashing](https://quotenil.com/adaptive-hash-talk.pdf), which
  focusses on making general purpose hash tables _faster and more
  robust_ at the same time.

  ## Theory vs Practice

  Hash table theory most concerns itself with the asymptotic
  ...
  - we don't pick hash functions randomly but fix the hash function
    for the lifetime of the hash table.
  ...
  Still, the idea that we can do better by adapting the hash function
  to the actual keys is key. Can we do that _online_, that is, while
  the hash table is being used? Potential performance gains come from
  improving the constant factors mentioned above by

  - having fewer collisions, and

      ![cache-friendliness-regret](blog-files/cache-friendliness-regret.png)
  ...
  2. When the hash table is grown to more than 32 keys and it must be
     ...
     longest common run of low-bits in the keys.

  3. If too many collisions, we switch to the previous default SBCL
     EQ-hash function that has been tuned for a long time.

  4. If too many collisions, we switch to Murmur, a general purpose
     hash. We could also go all the way to cryptographic hashes.

  In step 2, the hash function with the single shift fits the memory
  ...
  of the same type allocated in a loop.

  ![adaptive-hash-eq-put](blog-files/adaptive-hash-eq-put.png)

  In this figure, the red line is the adaptive hash.

  ### Adapting [EQUAL][dislocated] hash tables

  For composite keys, running the hash function is the main cost.
  Adaptive hashing does the following.
  ...
  ![adaptive-hash-string-put](blog-files/adaptive-hash-string-put.png)
...
(defpost @untangling-literate-programming
  ...
  A [literate program][lp] intersperses narrative and code chunks.
  From this, source code to be fed to the compiler is generated by a
  process called _tangling_, and documentation by _weaving_. The
  specifics of tangling vary, but by allowing complete reordering and
  textual combination of chunks, it lets the human narrative drive the
  exposition at the cost of introducing an additional step into the
  ...
  1. present code to human readers in pedagogical order with narrative
     added, and

  2. make changing code and its documentation together easy.

  The [advantages][lp-advantages] of literate programming follow from
  these desiderata.
  ...
  explored in more detail [here][lp-untangled].
  ...
  untangled LP faces the following potential problems.

  1. _Order mismatch_: Things like inline functions and global
     variables may need to be defined before use. So, code order tends
     to deviate from narrative order to some degree.

  2. _Reduced locality_: Our main tool to sync code and narrative is
  ...
  [pax-note]: https://melisgl.github.io/mgl-pax-world/mgl-pax-manual.html#MGL-PAX:NOTE%20MGL-PAX:MACRO
  [pax-emacs-func]: https://melisgl.github.io/mgl-pax-world/mgl-pax-manual.html#MGL-PAX:@EMACS-FUNCTIONALITY%20MGL-PAX:SECTION
  [pax-gendoc]: https://melisgl.github.io/mgl-pax-world/mgl-pax-manual.html#MGL-PAX:@GENERATING-DOCUMENTATION%20MGL-PAX:SECTION
  [pax-browse]: https://melisgl.github.io/mgl-pax-world/mgl-pax-manual.html#MGL-PAX:@BROWSING-LIVE-DOCUMENTATION%20MGL-PAX:SECTION
  [pax-mathjax]: https://melisgl.github.io/mgl-pax-world/mgl-pax-manual.html#MGL-PAX:@MATHJAX%20MGL-PAX:SECTION
  [slime]: https://github.com/slime/slime
  [sly]: https://github.com/joaotavora/sly

  [MGL-PAX][pax], a Common Lisp untangled LP solution, aims to
  ...
  1. _Order_
       ...
       - Common Lisp is quite relaxed about the order of function
         definitions, but not so much about DEFMACRO, DEFVAR,
         DEFPARAMETER, DEFCONSTANT, DEFTYPE , DEFCLASS, DEFSTRUCT,
         DEFINE-COMPILER-MACRO, SET-MACRO-CHARACTER,
         SET-DISPATCH-MACRO-CHARACTER, DEFPACKAGE. However, code order
  ...
  2. _Locality_
       ...
       - Lisp macros replace chunks in the rare, complex cases where a
         ...
         parenthesis).
           ...
           This constraint forces us to organize code into meaningful,
       ...
       - PAX's [NOTE][pax-note] is an extractable, named comment. NOTE
         can interleave with code within e.g. functions to minimize
         the distance between the logic and its documentation.

       - Also, PAX hooks into the development to provide easy
         navigation in the documentation tree.

  3. _Source code only view:_ PAX supports [hiding][pax-emacs-func]
     verbose documentation (sections, docstrings, comments) in the
     editor.

  4. _Generating documentation_

       - PAX extracts docstrings, NOTEs and combines them with
         narrative glue in DEFSECTIONs.

       - Documentation can be [generated][pax-gendoc] as static
         HTML/PDF files for offline reading or [browsed
         live][pax-browse] (in an Emacs buffer or via an in-built web
         server) during development.

       - LaTeX math is supported in both PDF and HTML (via
         [MathJax][pax-mathjax], whether live or offline).

  In summary, PAX accepts a minimal deviation in code/narrative order
